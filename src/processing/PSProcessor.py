...
from Config import PS_PROCESSORS, TOO_LONG, MAX_PROCESSES
...
class PSProcessor():
    ...
    def start(self):
        ...
        cursor.execute('CREATE TABLE PSProcessor (ticket TEXT PRIMARY KEY, pid TEXT, application TEXT, command TEXT, created REAL, expires REAL)')
        ...
        while(True):
            ...
            # self.remove_expired_requests(db)

            self.accept_requests(db)

            self.assign_requests(db)
    ...
    def accept_requests(self, processor_db: Connection) -> None:
        ...
        try:
            ...
            if(ticket):
                cursor = processor_db.cursor()
                cursor.execute(
                    'INSERT INTO PSProcessor (ticket, application, command, pid, processed, created, expires) VALUES (?, ?, ?, ?, ?, ? ,?)',
                    (ticket['id'], ticket['application_name'], ticket['command'], None, None, ticket['created'], ticket['expires'])
                )
        except Exception:
            #TODO: Log this
            pass

    def assign_requests(self, processor_db: Connection) -> None:
    ...
    def remove_expired_requests(self, processor_db: Connection) -> None:
